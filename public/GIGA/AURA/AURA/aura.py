...
BANNER = f"""
{PUR}  ╔══════════════════════════════════════════════╗
  ║  {WHT}A U R A{PUR}  ·  v0.4  ·  field protocol         ║
  ║  {DIM}folder = node · files = everything{PUR}           ║
  ╚══════════════════════════════════════════════╝{R}
"""

# ── Session ───────────────────────────────────────────────────────────────
SESSION = os.path.expanduser("~/.aura_v4.json")
...
def main():
    ...
    # Extract channel if present: --channel name
    channel = None
    if "--channel" in args:
    ...
    if cmd == "new":
        ...
        field = AuraField(path, name, channel=channel)
        field.join()
        time.sleep(0.3)
