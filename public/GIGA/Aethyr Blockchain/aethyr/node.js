...
const oracleFeeder   = require('./scripts/oracle_feeder');
...
// 6. Oracle feeder — pushes live prices on-chain every 60s
oracleFeeder.init({ rpcCall: rpc.rpcCall, getNonce: relayer.getNonce, record });

// 7. Consensus — now includes BTC commitment in every block
consensus.init({
  tg:          tg.tg,
  tgNow:       tg.tgNow,
...
// 10. Telegram bot — includes BCP commands
tg.init({
  consensusState:  consensus.state,
